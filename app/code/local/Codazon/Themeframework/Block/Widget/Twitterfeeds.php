<?php
class Codazon_Themeframework_Block_Widget_Twitterfeeds extends Mage_Core_Block_Template implements Mage_Widget_Block_Interface
{
	...
	protected $_sweet = false;

	public function getLastestTweets(){
		if(!$this->_sweet){
			$username = $this->getUserName()?$this->getUserName():'twitter';
			$limit = $this->getLimit()?$this->getLimit():2;
			$this->_sweet = json_decode($this->getLastestTweetsByUserName($username,$limit));
		}
		return $this->_sweet;
	}
	public function getLastestTweetsByUserName($username = 'twitter', $limit = 2){
		...
		try {
			$twitter = Mage::getSingleton('themeframework/twitterApi')->init($settings); //new TwitterAPIExchange($settings);
